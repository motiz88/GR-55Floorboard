...
{	
	...
	if ( data.at(37) != default_data.at(5) ){    // check if a valid GR-55B file
  QMessageBox *msgBox = new QMessageBox();
     ...
     for (int h=0;h<count;h++)
       {                
          temp = data.mid(a+43, 128);            // copy SMF 128 bytes
        	...
        	default_data.replace(293, 128, temp);            // replace GR55 address "02"
        	temp = data.mid(a+443, 100);           // copy SMF part1
        	temp.append(data.mid(a+559,28));       // copy SMF part2
        	default_data.replace(434, 128, temp);            // replace GR55 address "03"
        	temp = data.mid(a+587, 128);           // copy SMF part1
        	default_data.replace(575, 128, temp);            // replace GR55 address "04"
        	temp = data.mid(a+715, 86);            // copy SMF part1
        	default_data.replace(716, 86, temp);             // replace GR55 address "05"
        	temp = data.mid(a+859, 128);           // copy SMF part1
        	default_data.replace(815,128, temp);             // replace GR55 address "06"
        	temp = data.mid(a+987, 72);            // copy SMF part1
        	temp.append(data.mid(a+1075,56));      // copy SMF part2
        	default_data.replace(956, 128, temp);            // replace GR55 address "07"
        	temp = data.mid(a+1131, 128);          // copy SMF part1
        	default_data.replace(1097,128, temp);            // replace GR55 address "08"
        	temp = data.mid(a+1259, 58);           // copy SMF part1
        	temp.append(data.mid(a+1333,42));      // copy SMF part2
        	default_data.replace(1238, 100, temp);           // replace GR55 address "09"
        	temp = data.mid(a+1403, 128);          // copy SMF part1
        	default_data.replace(1351,128, temp);            // replace GR55 address "0A"
        	temp = data.mid(a+1531, 44);           // copy SMF part1
        	temp.append(data.mid(a+1591,84));      // copy SMF part2
        	default_data.replace(1492, 128, temp);           // replace GR55 address "0B"
        	temp = data.mid(a+1675, 128);          // copy SMF part1
        	default_data.replace(1633,128, temp);            // replace GR55 address "0C"        
