...
QString MidiTable::rangeToValue(Midi range, QString hex)
{
    ...
    {
        if(range.id.contains(hex))
        {
            valueStr = range.level.at(range.id.indexOf(hex)).name;
            if (valueStr.isEmpty()){ valueStr = range.level.at(range.id.indexOf(hex)).desc; };
        } else {
            bool ok;
            int valueHex =  hex.toInt(&ok, 16);
            valueStr = range.level.at(valueHex).name;
            if (valueStr.isEmpty()){ valueStr = range.level.at(valueHex).desc; };
        };
    };
    return valueStr;
};
