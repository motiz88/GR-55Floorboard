...
GR-55FxFloorBoard version Alpha 20110408: 08 Apr 2011
*****************************************************
 ...
 *<<<< NOTE THAT THE EDITOR IS NOT YET FUNCTIONAL AND IS AT ALPHA DESIGN STAGE ONLY 
 *DO NOT DISTRIBUTE ANY SAVED PATCH FILES GENERATED BY THE EDITOR UNTIL BETA STAGE HAS PROVEN 100% COMPATABILITY>>>>
  
 Here is an update on "where it's at"...

Most of the patch functions are now implemented, except for

System setting are not yet fully implemented, just the GK/Pedal settings to do.

The File system is now 99% compatible with the Roland Librarian. 

Preset patches are not displayed, because i can't find the memory address they reside in,
 and Roland have not yet supplied any specification documentation (still waiting 3 months on).

No Help file system yet, i am leaving that until last, so i don't have to backtrack an repeatedly redo stuff.

Patch Text Summary is not complete, but gives most of the patch data, some parameters need data conversion to text.
